...
if "selected_pdf" not in st.session_state:
    st.session_state.selected_pdf = None  # Može biti rečnik sa podacima o stranici i zakonu
...
if query:
    ...
    if count == 0:
        ...
        for idx, entry in enumerate(results):
            ...
            with col_pdf_link:
                st.markdown("<div style='height: 12px; background-color: #98FB98;'></div>", unsafe_allow_html=True)
                ...
                if st.button(btn_label, use_container_width=True, key=f"btn_{idx}"):
                    if is_active:
                        ...
                        # Čuvamo ceo entitet da bismo ga rendersovali van petlje
                        st.session_state.selected_pdf = entry  
                        st.session_state.viewing_page = entry["page"]
                        st.session_state.scroll_to_pdf = True
                    st.rerun()

        # ── 💡 NOVI IZDVOJENI PRIKAZ PDF-A (VAN PETLJE) ────────────────────────
        # Smešten je tačno ispod rezultata pretrage, čime se obezbeđuje čistiji layout.
        if st.session_state.selected_pdf is not None:
            ...
            st.markdown("---")
            # 📍 HTML Sidro (Anchor) koje služi kao meta za skrolovanje
            st.markdown("<div id='pdf-pogled-lokacija'></div>", unsafe_allow_html=True)
            ...
            if pdf_path and os.path.exists(pdf_path):
                try:
                    with fitz.open(pdf_path) as doc:
                        ...
                        # ✨ UNAPREĐEN JAVASCRIPT ZA CENTRIRANJE PDF-A NA EKRANU
