...
import streamlit.components.v1 as components  # Potrebno za JavaScript skrolovanje
import re
import os
            ...
            with col_pdf_link:
                ...
                if st.button(btn_label, use_container_width=True, key=f"btn_{idx}"):
                    if is_active:
                        ...
                        st.session_state.scroll_to_top = True  # Skroluj na vrh pri prvom otvaranju
                    st.rerun()

            # ── PRIKAZ SA DUPLOM NAVIGACIJOM I PAMETNIM SKROLOVANJEM ──────────
                ...
                if pdf_path and os.path.exists(pdf_path):
                    try:
                        with fitz.open(pdf_path) as doc:
                            ...
                            # ── JAVASCRIPT OKIDAČ ZA SKROLOVANJE NA VRH ──
                            if st.session_state.scroll_to_top:
                                components.html(
                                    """
                                    <script>
                                        try {
                                            var selectors = [".main", ".stMain", "[data-testid='stAppViewContainer']"];
                                ...
                                st.session_state.scroll_to_top = False # Odmah gasimo okidač do sledećeg klika
                            ...
                            with nav_prev:
                                if st.button("⬅ Prethodna strana", use_container_width=True, disabled=is_first, key=f"prev_top_{idx}"):
                            ...
                            with nav_next:
                                if st.button("Sledeća strana ➡", use_container_width=True, disabled=is_last, key=f"next_top_{idx}"):
                                    st.session_state.viewing_page += 1
                                    st.session_state.scroll_to_top = True
                                    st.rerun()
                            st.markdown("</div>", unsafe_allow_html=True)
                            ...
                            with nav_prev_b:
                                if st.button("⬅ Prethodna strana", use_container_width=True, disabled=is_first, key=f"prev_bot_{idx}"):
                                    st.session_state.viewing_page -= 1
                                    st.session_state.scroll_to_top = True
                                    st.rerun()
                            ...
                            with nav_next_b:
                                if st.button("Sledeća strana ➡", use_container_width=True, disabled=is_last, key=f"next_bot_{idx}"):
                                    st.session_state.viewing_page += 1
                                    st.session_state.scroll_to_top = True
                                    st.rerun()
                            st.markdown("</div>", unsafe_allow_html=True)
...
if st.session_state.about_open:
    st.markdown(
        ...
        <div class="about-card" style="border: 1px solid #ddd; padding: 20px; border-radius: 10px; background: #fff; margin-bottom: 20px;">
            <h2>📋 O aplikaciji</h2>
            <span class="about-version">Verzija 1.4</span>
...
st.markdown(
    ...
    "📋 Pretraga Pravilnika PPMV / PTPV &nbsp;·&nbsp; Verzija 1.4 &nbsp;·&nbsp; "
