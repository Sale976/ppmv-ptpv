...
import re
import os
import fitz  # PyMuPDF (obavezno dodati "PyMuPDF" u requirements.txt)
    ...
    if count == 0:
        ...
        for idx, entry in enumerate(results):
            ...
            with col_pdf_link:
                ...
                btn_label = "Zatvori prikaz" if is_active else f"📄 Prikaži str. {entry['page']}"
            ...
            # ── BEZBEDNO RENDEROVANJE STRANICE KAO SLIKE ──────────────────────
            if st.session_state.active_pdf_idx == idx:
                pdf_path = PDF_FILES.get(entry["law"])
                
                if pdf_path and os.path.exists(pdf_path):
                    try:
                        # Otvaramo PDF fajl pomoću PyMuPDF-a
                        with fitz.open(pdf_path) as doc:
                            # PDF stranice počinju od indeksa 0 (strana 1 je indeks 0)
                            target_page_idx = entry["page"] - 1
                            
                            if 0 <= target_page_idx < len(doc):
                                page = doc.load_page(target_page_idx)
                                # dpi=150 daje savršenu oštrinu teksta, a fajl ostaje lagan
                                pix = page.get_pixmap(dpi=150)
                                img_bytes = pix.tobytes("png")
                                
                                # Prikazujemo izvučenu stranicu kao čistu sliku visoke rezolucije
                                st.image(
                                    img_bytes, 
                                    caption=f"Dokument: {entry['law']} — Stranica: {entry['page']}", 
                                    use_container_width=True
                                )
                            else:
                                st.error(f"⚠️ Stranica {entry['page']} ne postoji u PDF-u (Dokument ima ukupno {len(doc)} strana).")
                    except Exception as e:
                        st.error(f" Greška pri otvaranju stranice dokumenta: {e}")
                else:
...
if st.session_state.about_open:
    st.markdown(
        ...
        <div class="about-card" style="border: 1px solid #ddd; padding: 20px; border-radius: 10px; background: #fff; margin-bottom: 20px;">
            <h2>📋 O aplikaciji</h2>
            <span class="about-version">Verzija 1.1</span>
...
st.markdown(
    ...
    "📋 Pretraga Pravilnika PPMV / PTPV &nbsp;·&nbsp; Verzija 1.1 &nbsp;·&nbsp; "
