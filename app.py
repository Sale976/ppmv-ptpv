...
# ── 💡 KORAK 1: REZERVISANO MJESTO NA SAMOM VRHU EKRANA ───────────────────────
# Ovo stvara nevidljivi kontejner na apsolutnom početku stranice
pdf_top_placeholder = st.empty()
...
# ── 💡 KORAK 2: DINAMIČKO UBACIVANJE PDF-A NA SAMI VRH STRANICE ────────────────
    ...
    if pdf_path and os.path.exists(pdf_path):
        try:
            with fitz.open(pdf_path) as doc:
                ...
                # Preusmjeravamo sav prikaz PDF-a unutar gornjeg praznog kontejnera
                with pdf_top_placeholder.container():
                    
                    # JS koji forsira skrolovanje čitavog monitora na apsolutnu nulu (sam vrh)
                    ...
                    st.markdown('</div>', unsafe_allow_html=True)
                    st.markdown("---")
                    st.markdown("### 🔍 Sekcija za pretragu i rezultate (pomjerena ispod otvorenog dokumenta):")
