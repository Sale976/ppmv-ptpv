...
<style>
    ...
    /* Kontejner oko celog PDF prikaza */
    .pdf-container {
...
# ── 💡 NOVI PREMEŠTENI PRIKAZ PDF DOKUMENTA (SADA JE NA SAMOM VRHU) ───────────
# Pozicioniranjem na vrh, eliminišemo problem gde dno stranice blokira skrolovanje čitača.
if query and st.session_state.selected_pdf is not None:
    ...
    # 📍 HTML Sidro postavljeno na samom vrhu ove sekcije
    ...
    if pdf_path and os.path.exists(pdf_path):
        try:
            with fitz.open(pdf_path) as doc:
                ...
                # Skript koji pomera prozor tako da vrh PDF okvira bude zalepljen za vrh monitora
                ...
                st.markdown("</div>", unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)
                st.markdown("### 🔍 Lista rezultata pretrage (ispod otvorenog dokumenta):")
