...
<style>
    ...
    /* Kontejner oko celog PDF prikaza za lakše skrolovanje */
    .pdf-container {
...
# Čuvamo selektovani PDF objekat umesto samo indeksa iz petlje da bismo ga izdvojili
...
if query:
    ...
    if count == 0:
        ...
        # ── 💡 IZDVOJENI PRIKAZ PDF-A SA APSOLUTNIM SKROLOVANJEM NA VRH ────────
        if st.session_state.selected_pdf is not None:
            ...
            st.markdown("---")
            # 📍 HTML Sidro koje služi kao nulta tačka za monitor
            st.markdown("<div id='pdf-pogled-lokacija' style='margin-top: 0px; padding-top: 0px;'></div>", unsafe_allow_html=True)
            ...
            if pdf_path and os.path.exists(pdf_path):
                try:
                    with fitz.open(pdf_path) as doc:
                        ...
                        # ✨ NOVI JS KOJI DIŽE POČETAK PDF-A NA SAM VRH MONITORA
                        if st.session_state.scroll_to_pdf:
                            components.html(
                                """
                                <script>
                                    try {
                                        var mada = window.parent.document.getElementById("pdf-pogled-lokacija");
                                        if (mada) {
                                            // Računamo tačnu udaljenost elementa od vrha roditeljskog dokumenta
                                            var krov_skrola = mada.getBoundingClientRect().top + window.parent.pageYOffset;
                                            
                                            // Skrolujemo ceo prozor tačno na izračunatu poziciju (0px od vrha ekrana)
                        ...
                        # Celu sekciju stavljamo u lep CSS okvir radi uočljivosti
                        ...
                        st.markdown('</div>', unsafe_allow_html=True) # Kraj pdf-kontejnera
